...
class FacturaForm(ModelForm):
    class Meta:
        ...
        widgets = {
            'comprobantes_relacionados': forms.SelectMultiple(
                attrs={'data-placeholder': "Elija uno o mas comprobantes"}),
    ...
    class Meta:
        ...
        widgets = {
            ...
            'descuento': forms.TextInput(attrs={'class': 'descuento_input', \
                                                ...
                                                'data-bv-numeric': 'true', \
                                                'data-bv-numeric-message': 'Ingrese solamente números'}),
            'articulo_personalizado': forms.Textarea(
                attrs={'class': 'articulo_personalizado_input', 'rows': 5, 'cols': 20, \
                       'data-bv-group': 'td', \
...
class ValoresReciboForm(Form):
    ...
    cheque_banco = forms.ModelChoiceField(queryset=Bancos.objects.all(),
                                          widget=forms.Select(attrs={'class': 'cheque_banco_select', \
                                                                     'data-bv-notempty-message': 'Debe ingresar el banco', \
                                                                     'data-bv-notempty': 'true', \
                                                                     'data-bv-group': '.seccion_cheque', \
                                                                     'data-bv-selector': '.cheque_banco_select'}),
    ...
    transferencia_banco_origen = forms.ModelChoiceField(queryset=Bancos.objects.all(), widget=forms.Select(
        attrs={'class': 'transferencia_banco_origen_select', \
               'data-bv-notempty-message': 'Debe ingresar el banco de origen de la transferencia', \
               'data-bv-notempty': 'true', \
               'data-bv-group': '.seccion_transferencia', \
               'data-bv-selector': '.transferencia_banco_origen_select'}),
    ...
    transferencia_numero_operacion = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'transferencia_numero_operacion_input', \
                                      'data-bv-notempty-message': 'Debe ingresar el número de operación', \
                                      'data-bv-notempty': 'true', \
                                      'data-bv-group': '.seccion_transferencia', \
                                      'data-bv-selector': '.transferencia_numero_operacion_input'}),
        required=False)
    transferencia_cuenta_destino = forms.ModelChoiceField(queryset=Cuentas_banco.objects.all(), widget=forms.Select(
        attrs={'class': 'transferencia_cuenta_destino_select', \
               'data-bv-notempty-message': 'Debe ingresar la cuenta de destino de la transferencia', \
               'data-bv-notempty': 'true', \
               'data-bv-group': '.seccion_transferencia', \
               'data-bv-selector': '.transferencia_cuenta_destino_select'}),
...
class OrdenPagoForm(ModelForm):
    que_hago_con_diferencia = forms.ChoiceField(widget=forms.RadioSelect(),
                                                choices=(('credito', 'CREDITO'), ('vuelto', 'VUELTO')),
                                                initial='credito')
...
class ValoresOPForm(Form):
    ...
    id_cheque_tercero = forms.IntegerField(widget=forms.HiddenInput(attrs={'class': 'id_cheque_tercero_hidden'}),
                                           required=False)
    tipo = forms.ChoiceField(choices=TIPOS_CHOICES, widget=forms.Select(attrs={'class': 'tipo_valor_select'}))
    cheque_numero = forms.CharField(widget=forms.TextInput(attrs={'class': 'cheque_numero_input', \
                                                                  'data-bv-notempty-message': 'Debe ingresar el número del cheque', \
    ...
    cheque_paguese_a = forms.CharField(widget=forms.TextInput(attrs={'class': 'cheque_paguese_a_input', \
                                                                     ...
                                                                     'data-bv-selector': '.cheque_paguese_a_input'}),
                                       required=False)
    transferencia_cuenta_origen = forms.ModelChoiceField(queryset=Cuentas_banco.objects.all(), widget=forms.Select(
        attrs={'class': 'transferencia_cuenta_origen_select', \
               'data-bv-notempty-message': 'Debe ingresar la cuenta de origen de la transferencia', \
               'data-bv-notempty': 'true', \
               'data-bv-group': '.seccion_transferencia', \
               'data-bv-selector': '.transferencia_cuenta_origen_select'}),
                                                         required=False)
    transferencia_cuenta_destino = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'transferencia_cuenta_destino', \
                                      'data-bv-notempty-message': 'Debe ingresar la cuenta de destino de la transferencia', \
                                      'data-bv-notempty': 'true', \
                                      'data-bv-group': '.seccion_transferencia', \
                                      'data-bv-selector': '.transferencia_cuenta_destino_select'}),
...
class ComprasTotalesProv(Form):
    ano = forms.ChoiceField(
        choices=((2012, 2012), (2013, 2013), (2014, 2014), (2015, 2015), (2016, 2016), (2017, 2017), (2018, 2018)))

class VentasTotalesClie(Form):
